...
if ( ! class_exists( 'ratchet_Plugin_Notices' ) ) {
	...
	class ratchet_Plugin_Notices {
		...
		public function set_notices() {
			...
			// DEBUG:
			// delete_user_meta( get_current_user_id(), 'ratchet-dismiss-plugin-notice_update' );
			// delete_user_meta( get_current_user_id(), 'ratchet-dismiss-plugin-notice_install' );
			...
			if ( $to_install ) {
				...
				if ( get_user_meta( get_current_user_id(), 'ratchet-dismiss-plugin-notice_install', true ) !== $check ) {
			...
			if ( $to_update ) {
				...
				if ( get_user_meta( get_current_user_id(), 'ratchet-dismiss-plugin-notice_update', true ) !== $check ) {
		...
		public function display( $key, $value, $message ) {
			...
			?>
			<div id="ratchet-plugin-<?php echo esc_attr( $key ); ?>-notice" class="<?php echo $class; ?>" data-notice-key="<?php echo esc_attr( $key ); ?>" data-notice-value="<?php echo esc_attr( $value ); ?>" data-security="<?php echo esc_attr( $nonce ); ?>" data-namespace="ratchet" data-dismiss="<?php echo esc_attr( $this->args['nag_dismiss'] ); ?>">
		...
		public function dismiss() {
			...
			update_user_meta(
				get_current_user_id(),
				'ratchet-dismiss-plugin-notice_' . $_POST['notice_key'],
				$_POST['notice_value']
			);
